...
double cosine_similarity_simple(Datum* v1, Datum* v2, int size){
  ...
  for (int i = 0; i < size; i++){
    ...
    //elog(INFO, "v1 %lf, v2 %lf, vmult %lf, scalar %lf", ((double)DatumGetFloat4(v1[i])), ((double)DatumGetFloat4(v2[i])), ((double)DatumGetFloat4(v1[i])) * ((double)DatumGetFloat4(v2[i])), scalar);
    sqDistanceV2 += ((double)DatumGetFloat4(v2[i]))*((double)DatumGetFloat4(v2[i]));
    sqDistanceV1 += ((double)DatumGetFloat4(v1[i]))*((double)DatumGetFloat4(v1[i]));
    count++;
  }
  if ((sqDistanceV1 > 0) && (sqDistanceV2 > 0)){
    double sim = (scalar / (sqrt(sqDistanceV1)*sqrt(sqDistanceV2)));
    //elog(INFO, "scalar %lf, sqD1 %lf, sqD2 %lf sim %lf, count %d", scalar, sqDistanceV1, sqDistanceV2, sim, count);
    return sim;
  }else{
    return 0;
  }
